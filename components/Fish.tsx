...
interface FishProps extends Omit<FishType, 'vx' | 'vy'> {}

const Fish: React.FC<FishProps> = ({ id, x, y, rotation, scale, color1, color2, isFlipped }) => {
  const style: React.CSSProperties = {
    position: 'absolute',
    left: 0,
    top: 0,
    transform: `translate(${x}px, ${y}px) rotate(${rotation}deg) ${isFlipped ? 'scaleY(-1)' : ''}`,
    width: 120 * scale,
    height: 50 * scale,
