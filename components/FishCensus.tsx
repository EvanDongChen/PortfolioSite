...
const FishCensus: React.FC<FishCensusProps> = ({ fishes, highlightedBehavior, onHighlightBehavior }) => {
  ...
  useEffect(() => {
    ...
    const handlePointerMove = (event: PointerEvent) => {
      ...
      const x = Math.min(Math.max(event.clientX - dragOffsetRef.current.x, margin), maxX);
      const y = Math.min(Math.max(event.clientY - dragOffsetRef.current.y, margin), maxY);
  ...
  return (
    <>
      <button
        ...
        aria-label={isOpen ? 'Close Fish Census' : 'Open Fish Census'}
        aria-pressed={isOpen}
      >
      ...
      {isOpen && (
        ...
        >
          <div
            ...
            className={`flex justify-between items-center mb-4 pb-2 border-b ${headerBorderClass} ${isDragging ? 'cursor-grabbing' : 'cursor-grab'}`}
          >
